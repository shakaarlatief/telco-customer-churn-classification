...
For interpretation, the selected model is the L2 logistic regression model with the highest cross-validated PR-AUC in the L2 grid. This selects \(C=1\). The model is then fitted on the full training set only for coefficient extraction. These fitted coefficients are not used to report in-sample performance.
