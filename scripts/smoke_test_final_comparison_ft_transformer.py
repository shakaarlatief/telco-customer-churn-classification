"""Training-only smoke test for the C25 FT-Transformer candidate.

The test validates C25 routing and mechanics on development training data only. It
...
def parameters_for_route(
    *,
    ...
    """Build a low-cost forced C25 S0 route."""
...
def assert_routes() -> None:
    """Verify C25's feature, selector, and weighted-only imbalance contracts."""
    if supported_feature_policies(CANDIDATE_FT_TRANSFORMER) != (
        FEATURE_POLICY_RAW,
        FEATURE_POLICY_DOMAIN,
    ):
        raise AssertionError("C25 must support exactly F0 and F1.")
    for feature_policy in (FEATURE_POLICY_RAW, FEATURE_POLICY_DOMAIN):
        if supported_feature_selection_policies(
            CANDIDATE_FT_TRANSFORMER,
            feature_policy,
        ) != (FEATURE_SELECTION_NONE,):
            raise AssertionError("C25 must remain S0-only.")
        if supported_imbalance_policies(
            CANDIDATE_FT_TRANSFORMER,
            feature_policy,
            FEATURE_SELECTION_NONE,
        ) != (IMBALANCE_NONE, IMBALANCE_CLASS_WEIGHT_BALANCED):
            raise AssertionError("C25 must expose only I0 and I1.")
...
def assert_probability_output(fitted_pipeline, X_validation: pd.DataFrame) -> None:
    ...
    if predictions.shape != (len(X_validation),) or not np.isin(predictions, [0, 1]).all():
        raise AssertionError("C25 returned invalid binary labels.")

    probabilities = np.asarray(fitted_pipeline.predict_proba(X_validation), dtype=float)
    if probabilities.shape != (len(X_validation), 2):
        raise AssertionError("C25 predict_proba must return two columns.")
    if not np.isfinite(probabilities).all():
        raise AssertionError("C25 returned non-finite probabilities.")
    if np.any((probabilities < 0.0) | (probabilities > 1.0)):
        raise AssertionError("C25 returned probabilities outside [0, 1].")
    if not np.allclose(probabilities.sum(axis=1), 1.0):
        raise AssertionError("C25 probabilities must sum to one.")

    scores, kind = extract_continuous_scores(fitted_pipeline, X_validation)
    scores = np.asarray(scores, dtype=float)
    if kind != "probability":
        raise AssertionError(f"C25 must expose probabilities, not {kind!r}.")
    if scores.shape != (len(X_validation),) or not np.isfinite(scores).all():
        raise AssertionError("C25 returned invalid continuous scores.")


def _unwrap_ft_classifier(
    fitted_pipeline,
) -> CloneSafeFeaturePolicyFTTransformerClassifier:
    classifier = fitted_pipeline.named_steps["classifier"]
    if isinstance(classifier, BalancedSampleWeightClassifier):
        classifier = classifier.estimator_
    if not isinstance(classifier, CloneSafeFeaturePolicyFTTransformerClassifier):
        raise AssertionError("C25 did not build an FT-Transformer classifier wrapper.")
    return classifier


def assert_native_string_preprocessor(fitted_pipeline, feature_policy: str) -> None:
    """Verify C25 uses the existing native categorical string preprocessor."""
    preprocessor = fitted_pipeline.named_steps["preprocessor"]
    if not isinstance(preprocessor, FeaturePolicyNativeCategoricalPreprocessor):
        raise AssertionError("C25 must use the native categorical preprocessor.")
    if preprocessor.categorical_dtype:
        raise AssertionError("C25 must receive categorical columns as strings.")
    ...
    if tuple(preprocessor.categorical_features) != tuple(expected_categoricals):
        raise AssertionError("C25 native preprocessor has the wrong categorical schema.")


def assert_ft_transformer_mechanics(
    fitted_pipeline,
    *,
    feature_policy: str,
    X_validation: pd.DataFrame,
) -> None:
    """Verify FT-Transformer mappings, fitted schema, and CPU-safe settings."""
    wrapper = _unwrap_ft_classifier(fitted_pipeline)
    ...
    if wrapper.device_ != "cpu" or wrapper.num_workers_ != 0:
        raise AssertionError("C25 must use CPU-safe FT-Transformer settings.")
    if tuple(wrapper.numeric_features) != numeric_features:
        raise AssertionError("C25 FT-Transformer wrapper has the wrong numeric schema.")
    if tuple(wrapper.categorical_features) != categorical_features:
        raise AssertionError(
            "C25 FT-Transformer wrapper has the wrong categorical schema."
        )
    if wrapper.classes_.tolist() != [0, 1]:
        raise AssertionError("C25 must expose binary classes_ after fitting.")

    if wrapper.numeric_mean_.shape != (len(numeric_features),):
        raise AssertionError("C25 numeric_mean_ has the wrong shape.")
    if wrapper.numeric_scale_.shape != (len(numeric_features),):
        raise AssertionError("C25 numeric_scale_ has the wrong shape.")
    if not np.isfinite(wrapper.numeric_mean_).all():
        raise AssertionError("C25 numeric_mean_ must be finite.")
    if not np.isfinite(wrapper.numeric_scale_).all() or np.any(
        wrapper.numeric_scale_ <= 0.0
    ):
        raise AssertionError("C25 numeric_scale_ must be finite and positive.")
    ...
    if wrapper.cat_cardinalities_ != expected_cardinalities:
        raise AssertionError("C25 categorical cardinalities do not match mappings.")
    ...
    unknown_frame.loc[:, first_categorical] = "__C25_UNKNOWN_CATEGORY__"
    ...
    if not np.all(encoded[:, 0] == 0):
        raise AssertionError("C25 must encode unknown validation categories as zero.")
...
def fit_and_check(
    *,
    ...
    """Fit one declared route and validate the C25 contract."""
...
def assert_rejections() -> None:
    """Ensure excluded C25 routes fail before fitting."""
    ...
    try:
        ...
        raise AssertionError("C25 must reject F2.")
    ...
    try:
        ...
        raise AssertionError("C25 must reject non-S0 feature selection.")
    ...
    for imbalance_policy in (
        ...
        try:
            ...
            raise AssertionError(f"C25 must reject {imbalance_policy}.")


def main() -> None:
    """Run C25's focused training-only smoke test."""
    validate_candidate_registry()
    assert_routes()
    ...
    fit_and_check(
        ...
        y_train=y_train,
        X_validation=X_validation,
        seed=RANDOM_STATE + 252,
    )
    assert_rejections()
    print("FT-Transformer final-comparison smoke test passed.")
